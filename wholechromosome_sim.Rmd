...
#Name output file
outputFile <- gsub(" ", "", paste("SimPermuteAll_", simulations, ".csv"))
```
...
for (chr in chrlist) {
  obs_counts <- c(obs_counts, c(sum((subset(allchrdata, Chromosome == chr))$Bias == "L")))
  obs_counts <- c(obs_counts, c(sum((subset(allchrdata, Chromosome == chr))$Bias == "P")))
  obs_counts <- c(obs_counts, c(sum((subset(allchrdata, Chromosome == chr))$Bias == "M")))
  obs_counts <- c(obs_counts, c(sum((subset(allchrdata, Chromosome == chr))$Bias == "F")))
}
...
for (sim in 1:simulations) {
  ...
  simallchrdata <- allchrdata[sample(nrow(allchrdata)), ]
  simallchrdata$simChromosome <- allchrdata$Chromosome
